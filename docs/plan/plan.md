...
7. ~~**Race conditions**: If the user submits two prompts rapidly, two hook invocations run concurrently. Cache writes are not atomic — last writer wins. Acceptable for now; use a lock file in Phase 3+ if needed.~~ **RESOLVED (2026-08-15)** — SHA-256 JSON cache entries are written to a PID-scoped temp file under `RLM_CACHE_DIR` and atomically renamed into place, so interrupted or concurrent writes cannot expose partial final files. Last-writer-wins remains intentional; a lock file is not needed.
